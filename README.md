...
fname 메타데이터와 pHash만으로 "이 장비에 자동화 가능한 반복 패턴이 존재할 가능성"을 사전 판정한다. 클릭 좌표 집중도, 세션 길이 변동계수(CV), 세션 간 화면 시퀀스 유사도(LCS) 세 가지 신호를 종합하여 판정한다.
...
세 가지 신호를 기반으로 자동화 가능한 반복 패턴의 존재 가능성을 판정한다.
...
| 범위 | 판정 | 해석 |
|------|------|------|
| > 70% | 높음 | 정형화된 버튼 조작 패턴 존재 가능성. 작업자가 반복적으로 동일한 UI 요소를 클릭하고 있다 |
| 40% ~ 70% | 중간 | 일부 정형화된 패턴과 비정형 클릭이 혼재. 추가 확인 필요 |
| < 40% | 낮음 | 탐색적/비정형 클릭 행동. 클릭 위치가 분산되어 있어 정형 패턴이 약하다 |
...
세 가지 신호를 종합하여 최종 판정을 내린다:

| 판정 | 조건 | 의미 |
|------|------|------|
| **높음** | 3개 신호 중 2개 이상 "높음" | Vision AI 투입 우선 대상. 자동화 가능한 반복 패턴이 존재할 가능성이 높다 |
| **낮음** | 3개 신호 전체 "낮음" | 자동화 패턴 미약. Vision AI 투입 대상에서 제외 가능 (단, 최종 판단은 사람이 한다) |
