...
| 0.3 ~ 0.85 | 중간 | 대체로 비슷하나 일부 세션에서 편차가 있다 |
| > 0.85 | 낮음 (불일정) | 업무 종류가 세션마다 다르다. 작업량의 편차가 크다 |

> **참고**: CV 임계값은 `cv_high_threshold` (기본 0.3)와 `cv_low_threshold` (기본 0.85)로 설정 가능하다. 트리아지와 일반 모드 모두 동일한 기본값을 사용한다.
...
| 0.4 ~ 0.7 | 중간 | 부분적으로 유사한 흐름이 있으나 변동이 있다 |
| < 0.4 | 낮음 | 세션마다 다른 화면 경로. 정형화된 업무 흐름이 약하다 |
...
| **중간** | CV 0.3 ~ 0.85 | 추가 확인 필요 |
| **낮음** | CV > 0.85 | 세션 간 편차가 큼 — 일반적인 수동 조작 패턴 |
